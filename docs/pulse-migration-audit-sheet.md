...
10. Before merging any `P12.x` follow-up PR, record explicit check-query evidence from `gh pr view ... statusCheckRollup` or `gh pr checks --watch` (preferred: `scripts/gh/wait-required-checks.sh <PR#> pulse-route-gate operator-smoke`).
