...
import { incidentAcknowledgePath } from "@/lib/incidents-action-contract";
...
export async function POST(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const response = await fetch(`${API_URL}${incidentAcknowledgePath(id)}`, {
      method: "POST",
    ...
    if (!response.ok) {
      return NextResponse.json({ error: "acknowledge failed" }, { status: response.status });
    }
