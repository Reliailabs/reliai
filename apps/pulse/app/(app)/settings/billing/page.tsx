...
export default async function BillingPage() {
  ...
  const organization = activeOrganizationId
    ? await authRequest<OrganizationRead>(`/api/v1/organizations/${activeOrganizationId}`).catch(() => null)
    : null;
  const usageQuota = activeOrganizationId
    ? await authRequest<UsageQuotaStatus>(`/api/v1/organizations/${activeOrganizationId}/usage-quota`).catch(() => null)
    : null;
