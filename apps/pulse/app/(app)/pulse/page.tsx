...
export default async function PulsePage({
  ...
  const newestEventTime =
    changeFeed?.changes?.[0]?.created_at ??
    incidents.items[0]?.updated_at ??
    traces.items[0]?.created_at ??
    deployments.items[0]?.created_at ??
    demoNow;
  const now = new Date(newestEventTime).getTime();
