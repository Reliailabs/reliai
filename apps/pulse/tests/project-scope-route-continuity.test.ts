...
test("regression operations detail route preserves explicit project scope", () => {
  ...
  assert.match(file, /const selectedProjectId = resolveStrictScopedProjectId\(projects, projectIdParam\)/);
  assert.match(file, /redirect\("\/operations\?error=project_scope_required"\)/);
...
test("operations incident and graph routes preserve explicit project scope", () => {
  ...
  const operationsRegression = read("app/(app)/operations/regressions/[regressionId]/page.tsx");
  ...
  assert.match(operationsIncident, /const selectedProjectId = resolveStrictScopedProjectId\(projects, projectIdParam\)/);
  assert.match(operationsIncident, /redirect\("\/operations\?error=project_scope_required"\)/);
  ...
  assert.match(operationsRegression, /const selectedProjectId = resolveStrictScopedProjectId\(projects, projectIdParam\)/);
  assert.match(operationsRegression, /redirect\("\/operations\?error=project_scope_required"\)/);
  assert.match(operationsGraph, /searchParams: Promise<\{ project_id\?: string \}>/);
  assert.match(operationsGraph, /const selectedProjectId = resolveStrictScopedProjectId\(projects, projectIdParam\)/);
  assert.match(operationsGraph, /redirect\("\/operations\?error=project_scope_required"\)/);
