...
type BillingRouteContract = {
  path: string;
  pulse_behavior: "full_billing_surface" | "checkout_route";
...
type BillingOwnershipContract = {
  status: "closed";
  decision: {
    ownership: "implemented_in_pulse";
    owner: string;
    target_phase: string;
...
test("F1 contract: billing ownership decision is explicit and owner-complete", () => {
  ...
  assert.equal(contract.decision.ownership, "implemented_in_pulse");
});

test("F1 contract: pulse billing routes reflect implemented parity behavior", () => {
  const contract = JSON.parse(readRepoFile("docs/pulse-billing-ownership.json")) as BillingOwnershipContract;
  ...
  assert.equal(settingsBilling?.pulse_behavior, "full_billing_surface");
  assert.equal(billingSuccess?.pulse_behavior, "full_billing_surface");
  assert.equal(checkout?.pulse_behavior, "checkout_route");
  ...
  assert.equal(pulseRouteExists("apps/pulse/app/api/billing/checkout/route.ts"), true);
});
