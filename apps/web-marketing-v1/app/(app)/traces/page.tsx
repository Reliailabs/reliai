    ...
    <div className="space-y-6">
      <header className="rounded-[28px] border border-zinc-300 bg-white px-6 py-6 shadow-sm">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <p className="text-xs uppercase tracking-[0.24em] text-secondary">Trace explorer</p>
        ...
        <div className="border-b border-zinc-200 px-6 py-5">
          ...
          <form className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-6">
            <input
              ...
              className="rounded-xl border border-zinc-300 bg-white px-3 py-2 text-sm text-primary outline-none ring-0 placeholder:text-zinc-400"
            />
            <select
              name="environment"
              defaultValue={filters.environment ?? ""}
              className="rounded-xl border border-zinc-300 bg-white px-3 py-2 text-sm text-primary outline-none"
            >
            ...
            <input
              ...
              placeholder="Model"
              className="rounded-xl border border-zinc-300 bg-white px-3 py-2 text-sm text-primary outline-none placeholder:text-zinc-400"
            />
            <input
              name="prompt_version"
            ...
            <div className="xl:col-span-6 flex flex-wrap items-center gap-3">
              ...
              {hasFilters ? (
                <Link
                  ...
                  className="rounded-xl border border-zinc-300 px-4 py-2 text-sm text-secondary transition hover:bg-zinc-50 hover:text-primary"
                >
                  Clear
                </Link>
              ) : null}
        ...
        {traces.items.length === 0 ? (
          <div className="px-6 py-12">
            <div className="rounded-[24px] border border-dashed border-zinc-300 bg-zinc-50 px-6 py-10">
              ...
              <h2 className="mt-4 text-xl font-semibold text-primary">No traces match these filters</h2>
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full border-collapse text-left">
              <thead className="bg-zinc-50 text-xs uppercase tracking-[0.18em] text-secondary">
                <tr>
              ...
              <tbody>
                {traces.items.map((trace) => (
                  <tr key={trace.id} className="border-t border-zinc-200 align-top">
                    ...
                    <td className="px-6 py-4">
                      <p className="text-sm font-medium text-primary">{trace.model_name}</p>
                      <p className="mt-1 text-sm text-secondary">{trace.model_provider ?? "provider n/a"}</p>
                    </td>
                    <td className="px-6 py-4 text-sm text-secondary">
        ...
        {traces.next_cursor ? (
          <div className="flex items-center justify-between border-t border-zinc-200 px-6 py-4">
            ...
            <Link
              ...
              className="rounded-xl border border-zinc-300 px-4 py-2 text-sm text-primary transition hover:bg-zinc-50"
            >
              Next page
            </Link>
          </div>
