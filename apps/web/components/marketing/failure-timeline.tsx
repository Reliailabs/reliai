...
export function FailureTimeline() {
  ...
  return (
    <div ref={ref} className="relative pl-6">
      <div className="absolute left-[17px] top-4 h-[calc(100%-2rem)] w-px bg-[linear-gradient(180deg,rgba(15,23,42,0.25),rgba(15,23,42,0.06))]" />
        ...
        {items.map((item, index) => {
          ...
          return (
            <div
              key={item.title}
              className={cn(
                "relative rounded-[24px] border border-zinc-300 bg-white/90 px-5 py-5 shadow-sm transition-all duration-700",
                visible ? "translate-y-0 opacity-100" : "translate-y-4 opacity-0",
              )}
              style={{ transitionDelay: `${index * 90}ms` }}
            >
              <div className="absolute -left-[25px] top-5 flex h-8 w-8 items-center justify-center rounded-full border border-zinc-300 bg-white shadow-sm">
