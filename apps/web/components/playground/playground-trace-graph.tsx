...
export function PlaygroundTraceGraph({ scenario, stage }: PlaygroundTraceGraphProps) {
  ...
  return (
    <Card className="rounded-[30px] border-zinc-300 p-6">
      <p className="text-xs uppercase tracking-[0.24em] text-steel">Trace graph preview</p>
      <h2 className="mt-2 text-2xl font-semibold text-ink">Execution path</h2>
            ...
            <div key={node.label} className="flex items-center gap-3">
              <div
                className={cn(
                  ...
                  !isFailure && !isGuardrail && "border-zinc-200 bg-white text-ink",
                )}
              >
              ...
              {index < nodes.length - 1 ? <ArrowRight className="h-4 w-4 text-steel" /> : null}
            </div>
          );
        })}
      </div>
      <div className="mt-6 grid gap-3 md:grid-cols-3">
        <div className="rounded-[22px] border border-zinc-200 px-4 py-4">
          <p className="text-xs uppercase tracking-[0.18em] text-steel">Slowest span</p>
          <p className="mt-2 text-sm font-medium text-ink">{visible ? scenario.slowestSpan : "n/a"}</p>
        </div>
        <div className="rounded-[22px] border border-zinc-200 px-4 py-4">
          <p className="text-xs uppercase tracking-[0.18em] text-steel">Token-heavy span</p>
          <p className="mt-2 text-sm font-medium text-ink">{visible ? scenario.tokenHeavySpan : "n/a"}</p>
        </div>
        <div className="rounded-[22px] border border-zinc-200 px-4 py-4">
          <p className="text-xs uppercase tracking-[0.18em] text-steel">Guardrail retry</p>
          <p className="mt-2 text-sm font-medium text-ink">{visible ? scenario.guardrailRetry : "n/a"}</p>
        </div>
      </div>
    </Card>
  );
