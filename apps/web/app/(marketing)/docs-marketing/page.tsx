  ...
  return (
    <main className={`${marketingContainerClass} ${marketingSectionLargeClass} pb-24`}>
      ...
      <div className="max-w-3xl">
        <p className="text-xs uppercase tracking-[0.28em] text-secondary">Docs</p>
      ...
      <section className={marketingSectionClass}>
        <h2 className="text-2xl font-semibold tracking-tight text-primary">How Reliai works</h2>
        <p className="mt-2 text-sm text-secondary">Reliai is built around a single operational loop.</p>
        <div className="mt-6 divide-y divide-zinc-100 rounded-xl border border-zinc-200 bg-white shadow-sm">
          {workflowSteps.map((step, i) => (
            <div key={step.label} className="flex items-start gap-4 px-6 py-4">
              <span className="mt-0.5 w-5 font-mono text-xs tabular-nums text-secondary">{i + 1}</span>
              <div>
                <span className="text-sm font-semibold text-primary">{step.label}</span>
                <span className="ml-2 text-sm text-secondary">{step.body}</span>
              </div>
            </div>
          ))}
        </div>
      ...
      {/* System Areas */}
      <section className={marketingSectionClass}>
        <h2 className="text-2xl font-semibold tracking-tight text-primary">System areas</h2>
        <div className="mt-6 grid gap-4 md:grid-cols-3">
          {systemAreas.map((area) => (
            <Card key={area.title} className={marketingCardClass}>
              <h3 className="text-base font-semibold text-primary">{area.title}</h3>
              <p className="mt-3 text-sm leading-7 text-secondary">{area.body}</p>
            </Card>
          ))}
        </div>
      </section>
      ...
      {/* AI in Reliai */}
      <section className={marketingSectionClass}>
        <h2 className="text-2xl font-semibold tracking-tight text-primary">AI in Reliai</h2>
        <p className="mt-2 text-sm text-secondary">
          Reliai uses AI to assist operators — not replace them.
        </p>
        <div className="mt-6 grid gap-4 md:grid-cols-2">
          <Card className={marketingCardClass}>
            <h3 className="text-sm font-semibold text-primary">AI is used for</h3>
          ...
          <Card className={marketingCardClass}>
            <h3 className="text-sm font-semibold text-primary">AI is NOT used for</h3>
            <ul className="mt-3 space-y-2">
              {aiDoesNot.map((item) => (
                <li key={item} className="flex items-start gap-2 text-sm text-secondary">
        ...
        <div className="mt-4 divide-y divide-zinc-100 rounded-xl border border-zinc-200 bg-white shadow-sm">
          {limits.map((l) => (
            <div key={l.label} className="flex items-center gap-3 px-6 py-3">
              <span className="text-sm font-medium text-primary">{l.label}</span>
              <span className="text-xs text-secondary">— {l.note}</span>
            </div>
          ))}
        </div>
      ...
      {/* Start Here */}
      <section className={marketingSectionClass}>
        <h2 className="text-2xl font-semibold tracking-tight text-primary">Start here</h2>
        <ul className="mt-4 space-y-2">
          {startHere.map((item) => (
            <li key={item.label}>
              ...
              <span className="ml-2 text-xs text-secondary">— {item.note}</span>
            </li>
          ))}
        </ul>
