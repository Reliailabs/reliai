...
test.describe("Page navigation workflow", () => {
  test("dashboard → incidents → detail workflow", async ({ page }) => {
    ...
    navigationMetrics.push({ from: "start", to: "/dashboard", duration: Date.now() - dashboardLoad });

    await page.click('a[href="/incidents"]');
    await page.waitForURL("**/incidents");
    const incidentsLoad = Date.now();
    navigationMetrics.push({ from: "/dashboard", to: "/incidents", duration: Date.now() - incidentsLoad });
