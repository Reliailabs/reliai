...
function tone(health: string) {
  if (health === "degraded") return "bg-red-500/10 text-red-400 border border-red-500/30";
  if (health === "stalled") return "bg-amber-500/10 text-amber-400 border border-amber-500/30";
  if (health === "healthy") return "bg-emerald-500/10 text-emerald-400 border border-emerald-500/30";
    ...
    <div className="min-h-full p-6 space-y-6">
      ...
      <div className="grid gap-4 lg:grid-cols-4">
        <div className="rounded-lg border border-zinc-800 bg-zinc-900 px-5 py-4">
          ...
          <Stat variant="xl">{pipeline.total_events_published}</Stat>
          <p className="mt-2 text-sm text-zinc-400">Total ingested trace events on the primary topic.</p>
        </div>
        <div className="rounded-lg border border-zinc-800 bg-zinc-900 px-5 py-4">
          <div className="flex items-center gap-2 text-zinc-400">
            <Activity className="h-4 w-4" />
            <p className="text-xs uppercase tracking-[0.18em] text-zinc-400">Recent rate</p>
          </div>
          <Stat variant="xl">{pipeline.recent_events_published}</Stat>
        ...
        <div className="rounded-lg border border-zinc-800 bg-zinc-900 px-5 py-4">
          ...
          <Stat variant="xl">{degradedCount}</Stat>
          <p className="mt-2 text-sm text-zinc-400">Consumers with recent errors or stalled progress.</p>
        </div>
        <div className="rounded-lg border border-zinc-800 bg-zinc-900 px-5 py-4">
          <div className="flex items-center gap-2 text-zinc-400">
            <ShieldAlert className="h-4 w-4" />
            <p className="text-xs uppercase tracking-[0.18em] text-zinc-400">DLQ</p>
          </div>
          <Stat>{pipeline.dead_letter_topic ?? "disabled"}</Stat>
        ...
        <div className="space-y-6">
          <Card className="p-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <SectionLabel>Operating rule</SectionLabel>
          ...
          <Card className="p-6">
            <SectionLabel>Recent window</SectionLabel>
